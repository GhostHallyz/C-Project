...
//保存长度自定义的随机生成的秘钥无符号字符串
unsigned char pass[260] = { 0x0 };
...
//随机生产长度为keylen字节的密钥
void RC4_key(int kl, unsigned char *pass) 
{
	int tmp = 0;
...
char* ByteToHex(const unsigned char* vByte, const int vLen)
{
	if (!vByte)
		return NULL;
	char* tmp = new char[vLen * 2 + 1]; // 一个字节两个十六进制码，最后要多一个''
	int tmp2;
...
int main(int argc, char *argv[])
{
	...
	int kl = 128;
	//printf("please imput the length of KEY (0< kl < 256): kl = ");
	//scanf_s("%d", &kl);
	RC4_key( kl, pass);

	char *result1 = NULL;
	char *result2 = NULL;
	result1 = Encrypt(source, (char*)pass);
	printf("result1=%s\n", result1);
	result2 = Decrypt(result1, (char*)pass);
	printf("result2=%s\n", result2);
	delete[]result1;
	delete[]result2;
	system("pause");

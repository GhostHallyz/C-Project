...
bool CSerialPort::InitPort(UINT portNo /*= 3*/, UINT baud /*= CBR_9600*/, char parity /*= 'N'*/,
	UINT databits /*= 8*/, UINT stopsbits /*= 1*/, DWORD dwCommEvents /*= EV_RXCHAR*/)
{

	/** 临时变量,将制定参数转化为字符串形式,以构造DCB结构 */
	char szDCBparam[50] = { 0x0 };
...
bool CSerialPort::openPort(UINT portNo)
{
	...
	m_hComm = CreateFileA(szPort,     //设备名,COM1,COM2等
		...
		0,                            //共享模式,0表示不共享
		NULL,                         //安全性设置,一般使用NULL
		OPEN_EXISTING,                //该参数表示设备必须存在(打开而不是创建)
		0,                            //同步方式
		0);
...
int CSerialPort::GetComPort()
{
	HKEY hKey;
	char portName[256] = { 0x0 }, commName[256] = { 0x0 };
		...
		NULL, KEY_READ, &hKey) == ERROR_SUCCESS) // 打开串口注册表
	{
		...
		while (1)
		{
			...
			if (::RegEnumValue(hKey, k, portName, &dwLong, NULL, NULL, (PUCHAR)commName,
				&dwSize) == ERROR_NO_MORE_ITEMS) // 枚举串口
				break;
			std::cout << commName <<std::endl;
			k++;
		}
		RegCloseKey(hKey);
		return 0;
...
int CSerialPort::GetComFromDescriptionInfo()
{
